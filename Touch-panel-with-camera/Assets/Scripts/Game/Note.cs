...
public class NormalNote : Note
{
    ...
    public override NoteState Process(float prevTime, float currentTime, CamInputManager manager)
    {
        ...
        leftTime = time - currentTime;//여기에 수치를 더하면.... 더 늦게 판정됨. 여기에 음수를 더하면.... 더 빨리 판정됨
        ...
        else if (leftTime <= GameManager.badRange)
        {
            ...
            float xMin = (position - scale * 0.5f) * (manager.Resolution.x - 1);
            float xMax = (position + scale * 0.5f) * (manager.Resolution.x - 1);
            for (int i = 0; i <= manager.touchesLastIndex; i++)
            {
                if (isAvailableTouch[i] && manager.touches[i].minX <= xMax && manager.touches[i].maxX >= xMin)
...
public class SlideNote : Note
{
    ...
    public override NoteState Process(float prevTime, float currentTime, CamInputManager manager)
    {
        ...
        if (leftTime < -GameManager.goodRange)//판정 범위 (-good)-(good)
        {
            processed = true;
            return NoteState.Bad;
        }
        else if (leftTime <= GameManager.goodRange)
        {
            float xMin = (position - scale * 0.5f) * (manager.Resolution.x - 1);
            float xMax = (position + scale * 0.5f) * (manager.Resolution.x - 1);
            for (int i = 0; i <= manager.touchesLastIndex; i++)
            {
                if (manager.touches[i].minX <= xMax && manager.touches[i].maxX >= xMin)
                //위치가 맞고
                {
...
public class JumpNote : Note
{
    ...
    public override NoteState Process(float prevTime, float currentTime, CamInputManager manager)
    {
        ...
        if (LeftTime < -GameManager.badRange)//판정 범위 (-bad)-(0)
        {
            processed = true;
            return NoteState.Bad;
        }
